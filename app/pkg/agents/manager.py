...
class ClientManager:
    ...
    def create(self, order_id: str, directory: str):
        client = Client(model=self.model, api_key=self.api_key, directory=directory, order_id=order_id)
        self.clients[order_id] = client
        self.queue.put(client)
        logger.info(f"Created new client for order {order_id}")
    ...
    def delete(self, order_id: str):
        if order_id in self.clients:
            ...
            logger.info(f"Deleted client for order {order_id}")
    ...
    async def _process_queue(self):
        while not self._stop_event.is_set():
            try:
                ...
                logger.info("Starting to process client task")
                try:
                    blob_id = await client.begin()
                    with self.app.app_context():
                        db.session.query(Order).filter_by(order_id=order_id).update({'blob_id': blob_id})
                        db.session.commit()
                    logger.info("Successfully completed client task", blob_id, order_id)

                except asyncio.CancelledError:
                    logger.warning("Task was cancelled")
                    raise
                except Exception as e:
                    logger.error(f"Error processing client task: {str(e)}", exc_info=True)
                finally:
                ...
                logger.info("Queue processor was cancelled")
                break
            except Exception as e:
